...
Rectangle {
    ...
    function generateRecommendedModelName() {
        // 生成推荐的模型名称
        let timestamp = new Date()
        let dateStr = timestamp.toISOString().slice(0, 10).replace(/-/g, '')
        let timeStr = timestamp.toTimeString().slice(0, 8).replace(/:/g, '')
        let taskPrefix = root.selectedTask ? root.selectedTask.toUpperCase() : "MODEL"
        
        return `${taskPrefix}-${dateStr}_${timeStr}`
    }
